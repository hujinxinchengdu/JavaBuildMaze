 ...
 * @version 1.0
 */
public class RoomMaze extends MazeData {

    private final int remainingWalls;
    private static final int d[][] = {{-1, 0}, {0, 1}, {1, 0}, {0, -1}};

    public RoomMaze(int N, int M, Player player, Position goalLoc, int remainingWalls) {
    ...
    public void builtMaze(MazeData data) {
        ...
        while (n < wallNeedToRemove) {
            ...
            if (current == WALL) {
                int xLine = isWall.get(positon).getX();
                int yLine = isWall.get(positon).getY();
                if (xLine == getN() - 1 || xLine == 0 || yLine == 0 || yLine == getM() - 1) {
                    continue;
                }
